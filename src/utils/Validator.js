...
class Validator {
  static validateCarName(carNameList) {
    if (carNameList.length === CAR_NAME.NO_INPUT_LENGTH) throw new NoInputError(ERROR_MESSAGE.NO_INPUT);
    ...
    if (carNameList.length === CAR_NAME.ONE_NAME) throw new OneNameError(ERROR_MESSAGE.ONE_NAME);
  }

  static validateRaceCount(raceCount) {
    if (raceCount === RACE_COUNT.NO_INPUT) throw new NoInputError(ERROR_MESSAGE.NO_INPUT);
    if (isNaN(raceCount)) throw new NotNumberError(ERROR_MESSAGE.NOT_NUMBER);
    if (raceCount < RACE_COUNT.MIN_VALUE) throw new InvalidMinNumberError(ERROR_MESSAGE.INVALID_MIN_NUMBER);
