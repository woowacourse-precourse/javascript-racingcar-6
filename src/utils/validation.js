...
const validation = {
  isValidLength(inputArr) {
    for (const el of inputArr) {
      if (el.length > 5 || el.length === 0) throw new Error(ERROR_MSG.INVALID_CAR_NAME_LENGTH);
    }
    return 0;
  },

  isDuplicated(inputArr) {
    if (new Set(inputArr).size !== inputArr.length) throw new Error(ERROR_MSG.DUPLICATED_CAR_NAME);
    return 0;
  },
  ...
  isTooBig(input) {
    ...
    if (input <= 0 || parseInt(input) > MOVE_MAX) throw new Error(ERROR_MSG.TOO_BIG_MOVE_COUNT);
    return 0;
  },
