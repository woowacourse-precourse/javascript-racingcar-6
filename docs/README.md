...
1. 경주할 자동차 이름을 입력 받는다.
   ...
   - [ ] Input에서 validator 체크
     ...
     - [ ] 자동차 이름 2자 이상 5자 이하 (최소 글자 길이는 정해진게 없기 때문에 2자로 [스스로 결정])
     - [ ] 자동차 최소 2대 이상부터 6대까지 (기록 레이싱이아닌 상대적 경쟁이기에 최소 2명이상 부터 [스스로 결정])
     - [ ] 자동차 이름에 사용할 수 있는 문자 한국어와 영어만 허용 (대신 나중에 언어를 확장할 수 있도록 구현하기[스스로 결정])
     - [ ] 자동차 이름을 중복으로 명명할 수 없다. ( 보통 닉네임은 중복허용하지 않음 [스스로 결정])

2. 시도할 횟수를 입력받는다.

   - [ ] InputView 입력 받기
   - [ ] Input에서 validator 체크
     - [ ] 횟수 1~9회 제한 (제한이 정해지지 않았기 때문에 스스로 결정, 무한으로 할 수 없기 때문에) (언제든 변경하기 쉽도록)

3. 입력한 횟수 만큼 자동차 경주를 한다.
   - [ ] '실행 결과' 메시지 출력
   - [ ] 1. 랜덤한 숫자를 생성한다. (특정 숫자 이상이면 전진을 하기 위해)
...
1. 경주할 자동차 이름을 입력 받는다.
   ...
   - [ ] InputView 입력 잘 받아지는지 확인
   - [ ] InputView에서 validator 체크
     - [ ] 쉼표 기준으로 구분되는지
     - [ ] 자동차 이름이 2자 이상 5자 이하인지
     - [ ] 자동차 최소 2대 이상부터 6대 이하
     - [ ] 자동차 이름에 한국어와 영어만 허용
     - [ ] 자동차 이름 중복 확인
