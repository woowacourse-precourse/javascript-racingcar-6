...
- [ ] DO : 사용자로부터 입력값을 받는다.
  - [ ] DO : 입력값의 공통 유효성을 검사한다.(비어있는 문자열에 대해 예외 처리를 진행한다.)
- [ ] DO : 사용자에게 경주할 자동차의 이름을 입력받는다.

```bash
경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)
```

- [ ] DO : 사용자에게 경주의 시도 횟수(round)를 입력받는다.
...
- [ ] DO : 각 라운드별로 자동차 이름과 전진 상태를 출력한다.
...
- [ ] DO : 라운드와 입력받은 자동차들로 게임을 초기화한다.

### Racing

- [ ] DO : 주어진 횟수 동안 게임을 진행한다.
- [ ] DO : 각 라운드마다 각 자동차는 전진하거나 멈춘다.
