...
- 입력 함수
    - 입력 판단
      - 5자 이하의 경우 : throw Error("[ERROR] 숫자가 잘못된 형식입니다.")
      - 그 외 : array push
    - 입력 : 횟수, 자동차 이름

    
- 전진 함수
    - 랜덤 숫자 발생 : MissionUtils.Random.pickNumberInRange(0, 9);
    - 전진 판단
    	- 0~3: 멈춤(자동차 이름 출력 안 함)
      - 4~9: 전진(자동차 이름 출력,n-4해서 전진 )
    - 전진 출력
    - 각 차수별 전진 점수 합산
    
- 우승자 출력
	- 한명 또는 2명 이상의 경우 모두 출력
   (ex) members.map((member)=>member).join(',')
