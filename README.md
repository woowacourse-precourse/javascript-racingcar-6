...
## 구현할 기능 목록
1. n개의 자동차 이름을 입력받고 배열에 넣는다.
2. 이동할 횟수를 입력받고 n크기의 이동한 거리 배열을 만든다.
3. 이동횟수만큼 n개의 자동차 각각 0~9까지의 랜덤한 수를 만들어
이동횟수 배열에 넣고, 배열 안의 값을 하이픈 개수로 출력한다.
4. 이동횟수가 끝나면 하이픈 수가 가장 많은 자동차의 이름을 출력한다.
