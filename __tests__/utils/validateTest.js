...
describe('validate 객체의 methods Test', () => {
  describe('메서드 test : carNames()', () => {
    test('입력값에 콤마(,)가 없을 경우 CarNamesError를 던짐', () => {
      ...
      expect(() => validate.carNames(userInput)).toThrow(CarNamesInputError);
  ...
  describe('메서드 테스트 : tryRound()', () => {
    test.each(['a3', '#1'])(
      '입력값에 숫자 이외의 값이 들어왔을때 TryRoundError를 던짐',
      (invalidInput) => {
        expect(() => {
          validate.tryRound(invalidInput);
        }).toThrow(TryRoundInputError);
        expect(() => {
          validate.tryRound(invalidInput);
    ...
    test.each(['+1', '-1'])(
      '입력값의 시작에 부호(+,-)가 넣었을때 TryRoundError를 던짐',
      (mathSignInput) => {
        expect(() => {
          validate.tryRound(mathSignInput);
        }).toThrow(TryRoundInputError);
        expect(() => {
          validate.tryRound(mathSignInput);
