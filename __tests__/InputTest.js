import { inputCarNames, inputAttemptNumber } from '../src/utils/inputUser.js';
...
describe('사용자 입력 테스트', () => {
    ...
    test('길이가 5를 넘기는 자동차 이름 입력 오류 ', async () => {
        ...
        await expect(inputCarNames()).rejects.toThrow('[ERROR] 이름은 5자 이하만 가능합니다.');
    });
    ...
    test('움직이는 시도가 정수가 아닌 때 오류', async () => {
        ...
        await expect(inputAttemptNumber()).rejects.toThrow('[ERROR] 시도할 횟수는 정수만 가능합니다.')
    })
})