...
describe("입력값 유효성 검사 테스트", () => {
  test("이름에 아무 값도 입력하지 않은 경우를 확인", () => {
    ...
    expect(() => {
      InputValidator.validateInputRaceCarName(nameInput);
    }).toThrow();
  });
  ...
  test("이름에 공백이 포함되어 있는 경우를 확인", () => {
    ...
    nameInput.forEach((name) => {
      expect(() => {
        InputValidator.validateInputRaceCarName(name);
      }).toThrow();
    });
  });
  ...
  test("횟수에 숫자가 아닌 경우를 확인", () => {
    ...
    triesInput.forEach((times) => {
      expect(() => {
        InputValidator.validateInputRaceCarNumberOfAttempts(times);
      }).toThrow();
    });
  });
