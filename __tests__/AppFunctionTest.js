...
describe('자동차 경주 게임 함수 기능 확인', () => {
  ...
  test('getInput: 자동차 이름 입력 받기', async () => {
    ...
    const result = await app.getInput('경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)\n');
  ...
  test('playGame: 게임 실행하기', async () => {
    ...
    expect(MissionUtils.Random.pickNumberInRange).toHaveBeenCalledTimes(times * mockCars.length);
  });
  ...
  test('displayWinner: 우승자 결과 출력(한 명인 경우)', () => {
    ...
    expect(MissionUtils.Console.print).toHaveBeenCalledWith('최종 우승자 : test1');
  });

  test('displayWinner: 우승자 결과 출력(다수인 경우)', () => {
    ...
    expect(MissionUtils.Console.print).toHaveBeenCalledWith('최종 우승자 : test1, test2');
  });
  ...
  test('validateName: 자동차 이름이 5글자를 초과할 경우, 예외처리', () => {
    ...
    expect(() => app.validateName(badInputNames)).toThrow('[ERROR] 자동차 이름은 5자 이하만 가능합니다.');
  });

  test('validateNumber: 경주할 횟수가 숫자가 아닌 경우, 예외처리', () => {
    const badInputNumber = 'abcd';

    expect(() => app.validateNumber(badInputNumber)).toThrow('[ERROR] 잘못된 값을 입력하였습니다.');
  });
});
